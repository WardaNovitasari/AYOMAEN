...
if(isset($_POST['submit'])){
	$id=$_POST['id'];
	$query1=mysqli_query($config,"SELECT * FROM tb_file_fiber WHERE id_form = '$id' AND id_tipe=1");
	...
	if(isset($_FILES['files1']['tmp_name'])){
    	...
    	if(in_array($file_ext, $allowed_ext)===true){
    		if($file_size < 1044070000){
    			...
    			echo '<script>alert("Data excel berhasil di update");window.location="../../user/home2.php?riwayat=2";</script>';
    		}else{
	...
	if(isset($_FILES['files2']['tmp_name'])){
    	...
    	if(in_array($file_ext2, $allowed_ext2)===true){
    		if($file_size2 < 1044070000){
    			...
    			echo '<script>alert("Data KMZ berhasil di update");window.location="../../user/home2.php?riwayat=2";</script>';
    		}else{
    			 echo '<div class="error">ERROR: Besar ukuran file (file size) maksimal 1 Mb!</div>';
    		}
    	}else{
