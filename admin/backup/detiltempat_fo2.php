       ...
       <?php include 'view/iconcards.php'; ?>
        ...
        <div class="card mb-3">
         ...
         <?php 
              ...
              $result=mysqli_query($config,"SELECT * FROM tb_tempat_fo WHERE id_form='$id'");
              ...
              for ($i=0; $i < $count; $i++) {
                $bersinggungan=0;
                $id2=$rows['id'];
                $perulangan1=mysqli_query($config,"SELECT * FROM tb_tempat_fo where id='$id1'");
                $rowsPer1=mysqli_fetch_array($perulangan1);
                $lokasi1_lat=$rowsPer1['lat'];
                $lokasi1_long=$rowsPer1['lng'];
                for ($j=0; $j < $count; $j++) { 
                  $perulangan2=mysqli_query($config,"SELECT * FROM tb_tempat_fo where id='$id2'");
                ...
                if($rowsPer1['status']=='terkirim' || $rowsPer1['status']=='ditolak'){
                  $id1++;
                  continue;
                }
                ?>
            <tr>
              ...
              <td>
                <center>
                  <a href="../aksi/acc_lokasi_fo.php?id=<?php echo $rowsPer1['id'];?>" class="smaller btn btn-success" title='terima' onclick="return confirm('Anda yakin mau menerima data ini ?')"><i class="fas fa-check"></i></a>&nbsp;
              ...
              <td>
                <a href="edit_detil_fo.php?id=<?php echo $rowsPer1['id'];?>" class="smaller btn btn-info" title='edit'><i class="fas fa-edit"></i></a>&nbsp;

                <a class="smaller btn btn-danger" href='../aksi/delete_lokasi_fo.php?id=<?php echo $rowsPer1['id'];?>' onclick="return confirm('Anda yakin mau menghapus tempat ini ?')"><i class="fas fa-trash"></i></a>
              </td>
            </tr>
                 <?php
                 include 'modal/modal_fo_saran.php';
            $id1++;
            // close while loop
              //
            }
